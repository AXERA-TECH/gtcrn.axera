...
class StreamGTCRNNoScatter(nn.Module):
    ...
    def forward(self, spec, 
        ...
        en_conv_cache_0 = en_conv_cache[:,:,:2,:]
        en_conv_cache_1 = en_conv_cache[:,:,2:6,:]
        en_conv_cache_2 = en_conv_cache[:,:,6:,:]
        de_conv_cache_0 = de_conv_cache[:,:,6:,:]
        de_conv_cache_1 = de_conv_cache[:,:,2:6,:]
        de_conv_cache_2 = de_conv_cache[:,:,:2,:]

        en_tra_cache_0 = en_tra_cache[:,0,:,:].reshape(1, 1, 16)
        en_tra_cache_1 = en_tra_cache[:,1,:,:].reshape(1, 1, 16)
        en_tra_cache_2 = en_tra_cache[:,2,:,:].reshape(1, 1, 16)
        de_tra_cache_0 = de_tra_cache[:,0,:,:].reshape(1, 1, 16)
        de_tra_cache_1 = de_tra_cache[:,1,:,:].reshape(1, 1, 16)
        de_tra_cache_2 = de_tra_cache[:,2,:,:].reshape(1, 1, 16)
        
        inter_cache_0 = inter_cache_0.reshape(1, 33, 16)
        inter_cache_1 = inter_cache_1.reshape(1, 33, 16)
...
if __name__ == "__main__":
    ...
    en_conv_cache = torch.zeros(1, 16, 16, 33).to(device)
    de_conv_cache = torch.zeros(1, 16, 16, 33).to(device)
    
    # TRA cache size is (1, B, channels*2) where channels = in_channels//2 = 8, so channels*2 = 16
    en_tra_cache = torch.zeros(1, 3, 1, 16).to(device)
    de_tra_cache = torch.zeros(1, 3, 1, 16).to(device)
    
    inter_cache_0 = torch.zeros(1, 1, 33, 16).to(device)
    inter_cache_1 = torch.zeros(1, 1, 33, 16).to(device)
