...
def update_caches(cache_dict, outputs):
    return {
...
def denoise_audio(model_path, input_audio_path, output_audio_path, 
    ...
    stft_end = time.time()
    stft_time = stft_end - stft_start
    print(f"STFT耗时: {stft_time:.4f} 秒")
    ...
    for i in tqdm(range(spec.shape[2]), desc="处理中"):
        ...
        cache_dict = update_caches(cache_dict, outputs)
    ...
    istft_end = time.time()
    istft_time = istft_end - istft_start
    all_time = time.time() - start_time
    print(f"ISTFT耗时: {istft_time:.4f} 秒")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='GTCRN ONNX 音频降噪')
    ...
    parser.add_argument('--output', type=str, 
                       default='test_wavs/demo_output_ax630.wav',
                       help='输出音频路径')
